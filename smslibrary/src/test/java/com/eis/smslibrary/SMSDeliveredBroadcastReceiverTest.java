...
public class SMSDeliveredBroadcastReceiverTest {
    ...
    @Mock SMSDeliveredListener listener;
    ...
    public void testConstructor_noErrors(){
        SMSDeliveredBroadcastReceiver receiver = new SMSDeliveredBroadcastReceiver(VALID_MESSAGE, listener);
    }

    @Test
    public void receiveMessage_okResult(){
        SMSDeliveredBroadcastReceiver rec = spy(new SMSDeliveredBroadcastReceiver(VALID_MESSAGE, listener));
    ...
    @Test
    public void receiveMessage_verifyResultErrorParameters(){
        SMSDeliveredBroadcastReceiver rec = spy(new SMSDeliveredBroadcastReceiver(VALID_MESSAGE, listener));
        doReturn(Activity.RESULT_CANCELED).when((BroadcastReceiver)rec).getResultCode();
        rec.onReceive(contextMock, null);
        verify(listener, times(1))
